...
import gg.moonflower.pollen.api.biome.modifier.selector.BiomeSelectionMode;
...
public final class BiomeModifierManager implements PollinatedPreparableReloadListener {
    ...
    /**
     * Represents a registered biome modifier.
     *
     * @since
     */
    public record Entry(BiomeSelectionMode biomeSelectionMode, List<BiomeSelector> selectors, List<BiomeModification> modifications) {
        ...
        /**
         * Gets all modifications for the specified application phase.
         *
         * @param phase The phase to look for
         * @return A stream of biome modifications that apply at that phase
         */
        public Stream<BiomeModification> getModificationsByPhase(ModifierApplicationPhase phase) {
            return this.modifications().stream().filter(modification -> modification.getApplicationPhase() == phase);
        }
    }
